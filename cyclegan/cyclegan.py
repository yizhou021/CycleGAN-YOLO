...
def sample_images(batches_done):
    ...
    save_image(fake_B, "images/%s/%s.png" % (opt.dataset_name, batches_done), normalize=False)
    # image_grid = torch.cat((fake_A), 1)
    save_image(fake_A, "images/%s/%s-1.png" % (opt.dataset_name, batches_done), normalize=False)


def predict_image():
    """Saves a generated sample from the test set"""
    data_list = list(val_dataloader)
    for i in range(0,len(data_list)):
        ...
        save_image(fake_B, "imagesout/%s/%s0000.jpg" % (opt.dataset_name, i), normalize=False)
        # image_grid = torch.cat((fake_A), 1)
        save_image(fake_A, "imagesout/%s/%s0000-1.jpg" % (opt.dataset_name, i), normalize=False)
